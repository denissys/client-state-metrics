...
function getAvailabilityMetricsPerHour(ClientState, callback) {
  ...
  if (ClientState.hour < new Date().getHours()) {
  	...
  	redisClient.hget(ClientState.key+'-consumed', 'hit', function(err, hits) {
      ...
      var ClientAvailability = model.newInstance();
  	  ...
  	  ClientAvailability.date                 = convertDate.getDate(ClientState.date, convertDate.Patterns.YYYYMMDD); //new Date();
