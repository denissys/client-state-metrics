...
// ok  - consolidate hits
// ok  - persist metrics
// nok - verify schedules events
// nok - consolidate HITs + Schedule Events

async function processMetrics(clientStateKey) {
...
function getAvailabilityMetricsPerHour(ClientState, callback) {
  ...
  if (ClientState.hour < new Date().getHours()) {
      ...
      var ClientAvailability = model.newInstance();
	    ...
	    ClientAvailability.maxHits           = settings.keepAliveMetrics.maxHitsPerHour;
	    ClientAvailability.clientHits        = hits;
	    ClientAvailability.percentOnlineTime = ((ClientAvailability.clientHits / ClientAvailability.maxHits) * 100).toFixed(2);
	    ClientAvailability.onlineTimeInMin   = Math.round(60 * (ClientAvailability.percentOnlineTime / 100));
